...
- **Merge strategies**: the default is the deterministic merge in `orchestrator/merge.py` (dedup via `difflib` similarity + location match, no LLM call). An opt-in LLM-driven merge (`orchestrator/llm_merge.py`, `--llm-merge` on the CLI) instead calls Claude with `prompts/orchestrator.md`'s PHASE 2 instructions, catching semantic duplicates worded very differently across agents that the deterministic dedup misses — it automatically falls back to the deterministic merge on any API error, malformed response, or validation failure, so a flaky LLM call never blocks producing a report.
