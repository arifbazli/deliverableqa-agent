...
- Delta/re-run mode (`compute_delta()` in `orchestrator/merge.py`), tuned against real two-pass Bedrock output. **Known limitation:** a finding that gets both heavily reworded *and* relabeled to a different section between runs can still show up as one "resolved" + one "new" instead of matching as "still open" — the similarity threshold was tuned against real data to minimize this without risking false-merging genuinely unrelated findings, but it isn't eliminated. The dashboard does not yet render the `delta` block if present in an API response — it's in the JSON (`findings.json` / `/api/jobs/{id}` result) but only visible today via the CLI's summary line or by reading the JSON directly.
...
**Known gaps:**
- The orchestrator's *default* merge/rank step is still plain Python, not an LLM call — `prompts/orchestrator.md`'s PHASE 2 instructions are only used by the opt-in `--llm-merge` path, not by default. This is a deliberate cost/latency tradeoff (see Merge strategies above), not an oversight.
- No handling for documents that don't map cleanly to the section-per-heading assumption `parse.py` makes (e.g. decks with no text-frame titles, scanned PDFs needing OCR) — these now fail with a clear `DocumentParseError` rather than silently producing an empty or wrong report, but they still aren't *parsed*.
- The dashboard doesn't render the delta section yet (see the delta/re-run note above).
